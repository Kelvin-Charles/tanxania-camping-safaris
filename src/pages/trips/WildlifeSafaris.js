...
const WildlifeSafaris = () => {
  ...
  // Get all wildlife packages
  const wildlifePackages = useMemo(() => {
    const packages = new Set();
    Object.values(safariPackages.categories).forEach(category => {
      category.packages
        .filter(pkg => pkg.categories.includes('Wildlife Safaris'))
  ...
  const sortedPackages = useMemo(() => {
    return [...wildlifePackages].sort((a, b) => {
      switch (sortBy) {
        case 'price-low':
  ...
  }, [sortBy, wildlifePackages]);

  return (
    <div className="all-trips-page">
      <div className="trips-hero">
        <h1>Wildlife Safaris in Tanzania</h1>
        <p>Experience the incredible wildlife of Tanzania in their natural habitat</p>
      </div>
