...
const CulturalTours = () => {
  ...
  // Get all cultural packages
  const culturalPackages = useMemo(() => {
    const packages = new Set();
    Object.values(safariPackages.categories).forEach(category => {
      category.packages
        .filter(pkg => pkg.categories.includes('Cultural Tours'))
  ...
  const sortedPackages = useMemo(() => {
    return [...culturalPackages].sort((a, b) => {
      switch (sortBy) {
        case 'price-low':
  ...
  }, [sortBy, culturalPackages]);

  return (
    <div className="all-trips-page">
      <div className="trips-hero">
        <h1>Cultural Tours in Tanzania</h1>
        <p>Immerse yourself in the rich cultural heritage and traditions of Tanzania's diverse communities</p>
      </div>
