...
const MountainClimbing = () => {
  ...
  // Get all mountain climbing packages
  const mountainPackages = useMemo(() => {
    const packages = new Set();
    Object.values(safariPackages.categories).forEach(category => {
      category.packages
        .filter(pkg => pkg.categories.includes('Mountain Climbing'))
  ...
  const sortedPackages = useMemo(() => {
    return [...mountainPackages].sort((a, b) => {
      switch (sortBy) {
        case 'price-low':
  ...
  }, [sortBy, mountainPackages]);

  return (
    <div className="all-trips-page">
      <div className="trips-hero">
        <h1>Mountain Climbing in Tanzania</h1>
        <p>Conquer Africa's highest peaks and experience breathtaking mountain adventures</p>
      </div>
