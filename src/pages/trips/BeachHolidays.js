...
const BeachHolidays = () => {
  ...
  // Get all beach packages
  const beachPackages = useMemo(() => {
    const packages = new Set();
    Object.values(safariPackages.categories).forEach(category => {
      category.packages
        .filter(pkg => pkg.categories.includes('Beach Holidays'))
  ...
  }, [sortBy, beachPackages]);

  return (
    <div className="all-trips-page">
      <div className="trips-hero">
        <h1>Beach Holidays in Tanzania</h1>
        <p>Discover pristine beaches and crystal-clear waters of Tanzania's coastline and islands</p>
      </div>
